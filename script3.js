...
(() => {
  ...
  // ===== INJETOR: roda no contexto da PÁGINA (não no sandbox do TM) =====
  (function injectSidimEngineIntoPage() {
    const TAG_ID = 'sidim-auto-engine';
    if (document.getElementById(TAG_ID)) return;

    const code = `
(function(){
  if (window.__SIDIM_ENGINE__) return;
  window.__SIDIM_ENGINE__ = true;

  // ==== Utils ====
  function sleep(ms){ return new Promise(r=>setTimeout(r,ms)); }
  function waitForSelector(sel, timeout=12000){
    return new Promise((resolve,reject)=>{
      const t0 = performance.now();
      (function loop(){
        const el = document.querySelector(sel);
        if (el) return resolve(el);
        if (performance.now()-t0>timeout) return reject(new Error('timeout '+sel));
  ...
  function onRouteChange(handler){
    const wrap = fn => function(){ const r = fn.apply(this, arguments); setTimeout(handler,0); return r; };
    ...
    document.addEventListener('click', (e)=>{
      ...
      if (href.startsWith('/prontuarioeletronico_mariana/atendimento/')) setTimeout(handler,0);
    }, true);
  }

  // ==== A) Navegar para SOAP quando abrir em Folha de rosto ====
  function goToSOAPIfOnFolha(){
    if (!/\\/prontuarioeletronico_mariana\\/atendimento\\/\\d+\\/folhaderosto$/.test(location.pathname)) return;
    const link = Array.from(document.querySelectorAll('nav a.nav-link'))
      .find(a => /\\/prontuarioeletronico_mariana\\/atendimento\\/\\d+\\/primaria$/.test(a.getAttribute('href')||''));
    if (link) link.click();
  }

  // ==== B) Selecionar CID Z00.0 no Autocomplete (MUI) ====
  async function selectCID(){
    if (!/\\/prontuarioeletronico_mariana\\/atendimento\\/\\d+\\/primaria$/.test(location.pathname)) return;

    try {
      await waitForSelector('.form-group.row > label.col-form-label', 15000);
      ...
      if ((input.value||'').toUpperCase().includes('Z00.0')) return; // já preenchido

      // limpar e digitar "Z00" com eventos reais
      input.focus();
      ...
      function findOption(){
        const opts = [];
        document.querySelectorAll('[role="listbox"], .MuiAutocomplete-listbox, .MuiAutocomplete-paper')
          .forEach(lb => lb.querySelectorAll('[role="option"], li').forEach(li => opts.push(li)));
        return opts.find(el => wanted.test((el.textContent||'').trim().toLowerCase()));
      }

      let opt = findOption();
      const t0 = performance.now();
      while (!opt && performance.now()-t0 < 5000) {
        await sleep(100);
        opt = findOption();
      }
      ...
      if (opt) {
        opt.click();
        console.log('[ENGINE] CID Z00.0 selecionado.');
        return;
      }
      ...
      press(input,'Enter');
      console.log('[ENGINE] CID fallback (↓, Enter).');
    } catch(e){
      console.warn('[ENGINE] selectCID erro:', e.message);
    }
  }

  // ==== C) Marcar checkbox e radio em /finalizar (clicando no LABEL) ====
  async function finalizeChecks(){
    if (!/\\/prontuarioeletronico_mariana\\/atendimento\\/\\d+\\/finalizar$/.test(location.pathname)) return;
    try {
      const cb = document.getElementById('_cbConduta_1');
      const lbCb = document.querySelector('label[for="_cbConduta_1"]');
      if (cb && lbCb && !cb.checked) { lbCb.click(); console.log('[ENGINE] Checkbox marcado.'); }

      const rd = document.getElementById('desfecho0');
      const lbRd = document.querySelector('label[for="desfecho0"]');
      if (rd && lbRd && !rd.checked) { lbRd.click(); console.log('[ENGINE] Radio marcado.'); }
    } catch(e){
      console.warn('[ENGINE] finalizeChecks erro:', e.message);
    }
  }
  ...
  async function run(){
    ...
    busy = true;
    try {
      if (lastPath !== location.pathname) { lastPath = location.pathname; console.log('[ENGINE] rota:', lastPath); }
      goToSOAPIfOnFolha();
      await selectCID();
